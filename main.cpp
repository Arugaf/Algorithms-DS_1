...
void Merge(T& array, T& buf, int from, int mid, int to, std::function<bool(const T2&lhs, const T2& rhs)> comparator) {
    int k = from;
    int i = from;
    int j = mid + 1;
...
template <typename T, typename T2>
void MergeSort(T& A, T& temp, int low, int high, std::function<bool(const T2& lhs, const T2& rhs)> comparator) {
    for (int m = 1; m <= high - low; m *= 2) {
        for (int i = low; i < high; i += 2*m) {
            ...
            Merge<T, T2>(A, temp, from, mid, to, comparator);
        }
    }
}
...
int main() {
    ...
    MergeSort<Array<Point>, Point>(array, buf, 0, array.GetSize() - 1,
